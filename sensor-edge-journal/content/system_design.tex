...
We implement the floating-point computation adopting the dot-product with hybrid custom floating-point\cite{nevarez2021accelerating}. The hardware dot-product is illustrated in \Fig{fig:dot_product}. This approach: (1) denormalizes input values, (2) executes computation with integer format for exponent and mantissa, and finally, (3) it normalizes the result into IEEE 754 format. 
...
The 6-bit FP representation enables hardware optimization in the mantissa multiplication. The 1-bit mantissa enables efficient multiply-accumulate (MAC) implementations by reducing the mantissa multiplication to an addition, see \fig{fig:multiplier}. This approach reduces latency, energy consumption, and resource utilization.
