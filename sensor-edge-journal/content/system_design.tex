...
The 6-bit FP representation enables hardware a optimization in the mantissa multiplication. The 1-bit mantissa enables efficient multiply-accumulate (MAC) implementations by reducing the mantissa multiplication to a multiplexed addition, see \fig{fig:multiplier}. This approach reduces latency, energy consumption, and resource utilization.
...
The Infinity and NaN special cases are not considered in this design since are not expected in ANN computation. For the subnormal case, the element-wise multiplication is disabled when having a zero entry and applies approximation when having subnormal mantissa. In the case of standard FP, subnormal numbers are not expected and the generated error is negligible. In the case of 6-bit FP, the 1-bit mantissa has one subnormal case, which is handled as a normalized case. This approximation simplifies the hardware design.
